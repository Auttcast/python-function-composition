...
def test_author_query():
  schema_query = f(data) > f.shape | f.at(lambda x: x.models) | f.map(lambda x: x.author) | list
  data_query = f(data) > f.at(lambda x: x.models) | f.map(lambda x: x.author) | list
...
def test_filter():
  data_query = f(data) > f.at(lambda x: x.models) | f.map(lambda x: x.author) | f.distinct | f.filter(lambda x: "deep" in x)| list
  assert data_query == ["deepseek-ai"]

@tracelog("test_reduce")
def test_reduce():
  s1 = f(data) > f.at(lambda x: x.models) | f.map(lambda x: x.downloads) | list
  data_query = f(data) > f.at(lambda x: x.models) | f.map(lambda x: x.downloads) | f.reduce2(lambda x, y: x+y, 0)
  assert data_query == sum(s1)

@tracelog("test_flatmap")
def test_flatmap():
  data_query = (f(data) > f.at(lambda x: x.models)
...
@tracelog("test_sort")
def test_sort():
  data_query = f(['z', 'x', 'a', 'b']) > f.sort | list
  assert data_query == ['a', 'b', 'x', 'z']

@tracelog("test_take")
def test_take():
  dist = f(data) > f.at(lambda x: x.models) | f.map(lambda x: x.author) | f.distinct | list
  data_query = f(dist) > f.take(3) | f.list
  assert data_query == (f(dist) > f.list)[0:3]

@tracelog("test_skip")
def test_skip():
  dist = f(data) > f.at(lambda x: x.models) | f.map(lambda x: x.author) | f.distinct | list
  data_query = f(dist) > f.skip(3) | f.list
  assert len(data_query) == len(dist[3:])
  assert data_query == dist[3:]

@tracelog("test_sort_by")
def test_sort_by():
  unsorted_authors = f(data) > f.at(lambda x: x.models) | f.map(lambda x: x.author) | list
  native_sorted_authors = sorted(unsorted_authors)
  fsorted_authors = f(data) > f.at(lambda x: x.models) | f.sort_by(lambda x: x.author) | f.map(lambda x: x.author) | list

  assert fsorted_authors == native_sorted_authors

@tracelog("test_group")
def test_group():
  group_by_author = (f(data) > f.at(lambda x: x.models)
...
def test_join():
  ...
  group_by_author = f(data) > f.at(lambda x: x.models) | f.group(lambda x: x.author) | f.tee
